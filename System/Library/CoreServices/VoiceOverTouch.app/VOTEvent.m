...
{
  v6.receiver = self;
  v6.super_class = (Class)&OBJC_CLASS___VOTEvent;
  v4 = -[VOTEvent init](&v6, "init");
  if (v4)
  {
...
+ (id)externalDeviceEventWithCommand:(id)a3 info:(id)a4
{
  id v5 = a4;
  id v6 = a3;
  v7 = -[VOTEvent initWithType:](objc_alloc(&OBJC_CLASS___VOTEvent), "initWithType:", 3LL);
  -[VOTEvent setCommand:](v7, "setCommand:", v6);

  -[VOTEvent setInfo:](v7, "setInfo:", v5);
  return v7;
}
...
+ (id)keyEventWithCommand:(id)a3 keyInfo:(id)a4
{
  return +[VOTEvent keyEventWithCommand:keyInfo:otherInfo:]( &OBJC_CLASS___VOTEvent,  "keyEventWithCommand:keyInfo:otherInfo:",  a3,  a4,  0LL);
}
...
- (VOTKeyInfo)keyInfo
{
  return (VOTKeyInfo *)-[VOTEvent objectForIndex:](self, "objectForIndex:", 102LL);
}
...
- (void)setInfo:(id)a3
{
  ...
  if (info)
  {
    -[AXIndexMap addObjectsFromIndexMap:](info, "addObjectsFromIndexMap:", a3);
  }

  else
  {
    id v5 = (AXIndexMap *)[a3 copyWithZone:0];
    id v6 = self->_info;
    self->_info = v5;
  }
...
- (void)setObject:(id)a3 forIndex:(unint64_t)a4
{
  ...
  if (!info)
  {
    id v8 = (AXIndexMap *)[objc_allocWithZone(AXIndexMap) init];
    id v9 = self->_info;
    self->_info = v8;
  ...
  -[AXIndexMap setObject:forIndex:](info, "setObject:forIndex:", v6, a4);
}

- (id)objectForIndex:(unint64_t)a3
{
  return -[AXIndexMap objectForIndex:](self->_info, "objectForIndex:", a3);
}
...
- (id)description
{
  ...
  info = self->_info;
  id v5 = (void *)objc_claimAutoreleasedReturnValue(-[VOTEvent command](self, "command"));
  id v6 = (void *)objc_claimAutoreleasedReturnValue( +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"VOTEvent:[%p] Type: %d. Info: %@ Command:%@",  self,  origin,  info,  v5));
