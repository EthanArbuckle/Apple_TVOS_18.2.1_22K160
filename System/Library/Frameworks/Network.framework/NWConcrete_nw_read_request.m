...
- (NWConcrete_nw_read_request)init
{
  ...
  v16.receiver = self;
  v16.super_class = (Class)&OBJC_CLASS___NWConcrete_nw_read_request;
...
- (void)dealloc
{
  ...
  if (progress_timer_source)
  {
    ...
    self->progress_timer_source = 0LL;
  }

  v4.receiver = self;
  v4.super_class = (Class)&OBJC_CLASS___NWConcrete_nw_read_request;
...
- (NSString)description
{
  ...
  return (NSString *)(id)CFStringCreateWithFormat( (CFAllocatorRef)*MEMORY[0x189604DB0],  0LL,  @"<nw_read_request %zu/[%zu-%zu]%s>",  self->progress,  self->min,  self->max,  v2);
}
