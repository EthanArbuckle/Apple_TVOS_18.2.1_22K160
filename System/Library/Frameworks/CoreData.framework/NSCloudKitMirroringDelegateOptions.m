...
{
  v4.receiver = self;
  v4.super_class = (Class)&OBJC_CLASS___NSCloudKitMirroringDelegateOptions;
  v2 = -[NSCloudKitMirroringDelegateOptions init](&v4, sel_init);
  if (v2)
  {
    *((void *)v2 + 10) = objc_alloc_init(&OBJC_CLASS___PFCloudKitContainerProvider);
    *((void *)v2 + 11) = objc_alloc_init(&OBJC_CLASS___PFCloudKitStoreMonitorProvider);
    *((void *)v2 + 9) = objc_alloc_init(&OBJC_CLASS___PFCloudKitMetricsClient);
    *(_WORD *)(v2 + 13) = 1;
    *(_DWORD *)(v2 + 9) = 0;
    v2[15] = 0;
    *((void *)v2 + 12) = objc_alloc_init(&OBJC_CLASS___PFCloudKitMetadataPurger);
    *((void *)v2 + 14) = 0LL;
    *((void *)v2 + 15) = 2LL;
    *((void *)v2 + 17) = objc_alloc_init(&OBJC_CLASS___PFCloudKitArchivingUtilities);
    v2[18] = 1;
    v2[16] = 0;
    *((void *)v2 + 18) = objc_alloc_init(MEMORY[0x189603F18]);
    if (objc_msgSend( (id)objc_msgSend((id)objc_msgSend(MEMORY[0x1896079D8], "processInfo"), "processName"),  "isEqualToString:",  @"homed")) {
      v2[16] = 1;
    }
  }
...
- (NSCloudKitMirroringDelegateOptions)initWithContainerIdentifier:(id)a3
{
  objc_super v4 = -[NSCloudKitMirroringDelegateOptions init](self, "init");
  if (v4) {
...
- (id)copy
{
  ...
  *((void *)v3 + 3) = -[NSString copy](self->_containerIdentifier, "copy");
  *((void *)v3 + 4) = -[NSNumber copy](self->_ckAssetThresholdBytes, "copy");
...
- (void)dealloc
{
  ...
  self->_defaultOperationConfiguration = 0LL;
  objc_storeWeak((id *)&self->_progressProvider, 0LL);

  self->_archivingUtilities = 0LL;
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___NSCloudKitMirroringDelegateOptions;
...
{
  v32.receiver = self;
  v32.super_class = (Class)&OBJC_CLASS___NSCloudKitMirroringDelegateOptions;
  objc_super v3 = (void *)objc_msgSend(-[NSCloudKitMirroringDelegateOptions description](&v32, sel_description), "mutableCopy");
  ...
  if (!ckAssetThresholdBytes) {
    ckAssetThresholdBytes = (NSNumber *)[MEMORY[0x189603FE8] null];
  }
  ...
  if (!operationMemoryThresholdBytes) {
    operationMemoryThresholdBytes = (NSNumber *)[MEMORY[0x189603FE8] null];
  }
  ...
  if (!apsConnectionMachServiceName) {
    apsConnectionMachServiceName = (NSString *)[MEMORY[0x189603FE8] null];
  }
  [v3 appendFormat:@" apsConnectionMachServiceName:%@", apsConnectionMachServiceName];
  containerProvider = self->_containerProvider;
  if (!containerProvider) {
    containerProvider = (PFCloudKitContainerProvider *)[MEMORY[0x189603FE8] null];
  }
  ...
  if (!defaultOperationConfiguration) {
    defaultOperationConfiguration = (CKOperationConfiguration *)[MEMORY[0x189603FE8] null];
  }
  v24 = (void *)MEMORY[0x186E3E5D8]( [v3 appendFormat:@" defaultOperationConfiguration:%@", defaultOperationConfiguration]);
  id WeakRetained = objc_loadWeakRetained((id *)&self->_progressProvider);
  v26 = WeakRetained;
  if (!WeakRetained) {
    id WeakRetained = (id)[MEMORY[0x189603FE8] null];
  }
...
- (void)setInitializeSchema:(BOOL)a3
{
  ...
  uint64_t v3 = [NSString stringWithUTF8String:"NSCloudKitMirroringDelegateOptions.initializeSchema is no longer supported and will be removed in a future rele ase. Please use -[NSPersistentCloudKitContainer initializeSchemaWithOptions:error:] or NSCloudKitMirroringIniti alizeSchemaRequest instead."];
  _NSCoreDataLog(17LL, v3, v4, v5, v6, v7, v8, v9, v11);
  v10 = (os_log_s *)__pflogFaultLog;
  if (os_log_type_enabled((os_log_t)__pflogFaultLog, OS_LOG_TYPE_FAULT))
  {
    LOWORD(v11) = 0;
    _os_log_fault_impl( &dword_186681000,  v10,  OS_LOG_TYPE_FAULT,  "CoreData: NSCloudKitMirroringDelegateOptions.initializeSchema is no longer supported and will be removed in a futu re release. Please use -[NSPersistentCloudKitContainer initializeSchemaWithOptions:error:] or NSCloudKitMirroringI nitializeSchemaRequest instead.",  (uint8_t *)&v11,  2u);
  }
...
- (void)setActivityVouchers:(id)a3
{
  ...
  if (activityVouchers != a3)
  {
    ...
    {
      if (!-[NSArray count](activityVouchers, "count")) {
        return;
      }

      uint64_t v6 = (NSArray *)objc_alloc_init(MEMORY[0x189603F18]);
    }
