...
- (uint64_t)addObserver:()NSKeyValueObserverRegistration forKeyPath:options:context:
{
  ...
  return -[NSOrderedSet(NSKeyValueObserverRegistration) removeObserver:forKeyPath:context:]();
}
...
- (uint64_t)removeObserver:()NSKeyValueObserverRegistration forKeyPath:context:
{
  ...
  objc_exception_throw(v4);
  return -[NSOrderedSet(NSKeyValueObserverRegistration) removeObserver:forKeyPath:]();
}
...
- (uint64_t)removeObserver:()NSKeyValueObserverRegistration forKeyPath:
{
  ...
  objc_exception_throw(v4);
  return -[NSSet(NSKeyValueObserverRegistration) addObserver:forKeyPath:options:context:]();
}
