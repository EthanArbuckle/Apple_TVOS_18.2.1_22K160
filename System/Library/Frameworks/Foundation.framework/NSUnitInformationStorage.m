...
+ (void)initialize
{
  if (&OBJC_CLASS___NSUnitInformationStorage == a1) {
    objc_opt_class();
  }
}
...
+ (id)baseUnit
{
  return +[NSUnitInformationStorage bytes](&OBJC_CLASS___NSUnitInformationStorage, "bytes");
}
...
+ (id)_measurementWithNaturalScale:(id)a3 system:(unint64_t)a4
{
  if (qword_18C64DEE0 != -1) {
    dispatch_once(&qword_18C64DEE0, &__block_literal_global_700);
  }
  v5 = (void *)objc_msgSend( a3,  "measurementByConvertingToUnit:",  +[NSUnitInformationStorage bytes](NSUnitInformationStorage, "bytes"));
  v6 = -[NSDimension converter]( +[NSUnitInformationStorage bytes](&OBJC_CLASS___NSUnitInformationStorage, "bytes"),  "converter");
  [v5 doubleValue];
  double v8 = v7;
  -[NSUnitConverter coefficient](v6, "coefficient");
  if (a3 && v8 < v9) {
    return a3;
  }
  uint64_t v11 = [a3 unit];
  int v12 = [(id)qword_18C64DED8 containsObject:v11];
  if (v12) {
    uint64_t v13 = 1024LL;
  }
  else {
