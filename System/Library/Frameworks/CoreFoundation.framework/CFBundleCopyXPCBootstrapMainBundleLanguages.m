...
void ___CFBundleCopyXPCBootstrapMainBundleLanguages_block_invoke()
{
  ...
  if (v0)
  {
    ...
    if (value)
    {
      ...
      {
        v4 = v3;
        if (object_getClass(v3) == (Class)MEMORY[0x1895F9220])
        {
          size_t count = xpc_array_get_count(v4);
          _CFBundleCopyXPCBootstrapMainBundleLanguages_result = (uint64_t)CFArrayCreateMutable( 0LL,  count,  &kCFTypeArrayCallBacks);
...
uint64_t ___CFBundleCopyXPCBootstrapMainBundleLanguages_block_invoke_2(int a1, int a2, id a3)
{
  if (object_getClass(a3) == (Class)MEMORY[0x1895F92E8])
  {
    ...
    CFStringRef v6 = CFStringCreateWithBytes(0LL, (const UInt8 *)string_ptr, length, 0x8000100u, 0);
    if (v6)
    {
      CFStringRef v7 = v6;
