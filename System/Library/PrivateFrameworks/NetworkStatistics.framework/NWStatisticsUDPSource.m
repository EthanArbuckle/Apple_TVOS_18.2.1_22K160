...
- (id)_currentSnapshot
{
  return  -[NWSUDPSnapshot initWithCounts:UDPDescriptor:sourceIdent:events:seqno:provider:]( objc_alloc(&OBJC_CLASS___NWSUDPSnapshot),  "initWithCounts:UDPDescriptor:sourceIdent:events:seqno:provider:",  -[NWStatisticsSource _nstat_counts](self, "_nstat_counts"),  &self->_descriptor,  -[NWStatisticsSource reference](self, "reference"),  self->_eventFlags,  -[NWStatisticsSource snapshotRevision](self, "snapshotRevision"),  -[NWStatisticsSource provider](self, "provider"));
}
...
- (id)description
{
  ...
  asxprintf( &__ret,  v3,  0LL,  "NWSUDPSource %N<->%N@%I",  &self->_descriptor.local,  &self->_descriptor.remote,  self->_descriptor.ifindex);
  if (!__ret) {
    return 0LL;
  }
...
- (NWStatisticsUDPSource)initWithManager:(id)a3 local:(const sockaddr *)a4 remote:(const sockaddr *)a5
{
  ...
  id v8 = a3;
  v24.receiver = self;
  v24.super_class = (Class)&OBJC_CLASS___NWStatisticsUDPSource;
  v9 = -[NWStatisticsSource initWithManager:source:provider:](&v24, sel_initWithManager_source_provider_, v8, 0LL, 4LL);
  if (!v9) {
    goto LABEL_5;
  }
  __int128 v40 = 0u;
...
LABEL_5:
  ...
  if (os_log_type_enabled(v12, OS_LOG_TYPE_INFO))
  {
    ...
    *(_DWORD *)buf = 134219266;
    ...
    __int16 v35 = 2096;
    v36 = a5;
    _os_log_impl( &dword_187FF5000,  v12,  OS_LOG_TYPE_INFO,  "Manager %p: %s UDP source for local address %{network:sockaddr}.*P, remote address %{network:sockaddr}.*P",  buf,  0x36u);
  }
