...
- (id)debugDescription
{
  return (id)[objc_alloc(NSString) initWithFormat:@"%@:%d,%d (0x%llx-0x%llx)", self->_filePath, self->_lineNum, self->_columnNum, self->_offsetIntoSegment, self->_length + self->_offsetIntoSegment];
}
