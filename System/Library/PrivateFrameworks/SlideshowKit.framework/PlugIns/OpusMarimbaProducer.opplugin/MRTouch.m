...
+ (id)touchWithLocation:(CGPoint)a3 timestamp:(double)a4 tapCount:(unint64_t)a5
{
  CGFloat y = a3.y;
  CGFloat x = a3.x;
  v9 = objc_alloc_init(&OBJC_CLASS___MRTouch);
...
{
  v15.receiver = self;
  v15.super_class = (Class)&OBJC_CLASS___MRTouch;
  v6 = -[MRTouch init](&v15, "init");
  if (v6)
  {
...
+ (id)touchWithUITouch:(id)a3 inView:(id)a4
{
  return -[MRTouch initWithUITouch:inView:](objc_alloc(&OBJC_CLASS___MRTouch), "initWithUITouch:inView:", a3, a4);
}

- (void)dealloc
{
  self->_gestureRecognizers = 0LL;
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___MRTouch;
  -[MRTouch dealloc](&v3, "dealloc");
}

- (NSArray)gestureRecognizers
{
  return +[NSArray arrayWithArray:](&OBJC_CLASS___NSArray, "arrayWithArray:", self->_gestureRecognizers);
}
...
- (void)addGestureRecognizer:(id)a3
{
  ...
  {
    gestureRecognizers = objc_alloc_init(&OBJC_CLASS___NSMutableArray);
    self->_gestureRecognizers = gestureRecognizers;
  }

  -[NSMutableArray addObject:](gestureRecognizers, "addObject:", a3);
}
...
{
  v6.receiver = self;
  v6.super_class = (Class)&OBJC_CLASS___MRTouch;
  id v3 = -[MRTouch description](&v6, "description");
  unsigned int phase = self->_phase;
  if (phase >= 5) {
    unsigned int phase = 5;
  }
  return [v3 stringByAppendingFormat:@" location=(%f,%f), timestamp=%f, tapCount=%d, phase=%@, numberOfGestureRecognizers=%d", *(_OWORD *)&self->_location, *(void *)&self->_timestamp, self->_tapCount, off_24DF10[phase], -[NSMutableArray count](self->_gestureRecognizers, "count")];
}
