...
+ (id)keyPathsForValuesAffectingValueForKey:(id)a3
{
  if ([a3 isEqualToString:@"builtVolume"]) {
    return +[NSSet setWithObjects:]( &OBJC_CLASS___NSSet,  "setWithObjects:",  @"volume",  @"fadeInDuration",  @"fadeOutDuration",  0LL,  v7);
  }
  ...
  return +[NSSet setWithObjects:]( &OBJC_CLASS___NSSet,  "setWithObjects:",  @"asset",  @"startTime",  @"duration",  v6,  v7);
}

- (MCSong)init
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___MCSong;
  result = -[MCSong init](&v3, "init");
  if (result) {
    result->mVolume = 1.0;
  }
  return result;
...
{
  v23.receiver = self;
  v23.super_class = (Class)&OBJC_CLASS___MCSong;
  v6 = -[MCObject initWithImprint:andMontage:](&v23, "initWithImprint:andMontage:");
  uint64_t v7 = v6;
  if (v6)
  {
    ...
    -[MCObject addSong:](v9, "addSong:", v7);
    ...
    v7->mStartTimeIsDefined = v18 != 0LL;
    if (v18)
    {
      [v18 doubleValue];
      v7->mStartTime = v19;
...
- (void)demolish
{
  if (self->mAsset)
  {
    ...
    if (!-[MCObject isSnapshot](self, "isSnapshot")) {
      -[MCAssetAudio removeSong:](self->mAsset, "removeSong:", self);
    }
  ...
  self->mAudioPlaylistIfAudioPlaylistSong = 0LL;
  self->mSlideIfSlideSong = 0LL;
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___MCSong;
...
{
  v11.receiver = self;
  v11.super_class = (Class)&OBJC_CLASS___MCSong;
  id v3 = -[MCObject imprint](&v11, "imprint");
  mAsset = self->mAsset;
  if (mAsset)
  {
    v5 = -[MCObject objectID](mAsset, "objectID");
    v6 = self->mAsset;
    if (v5)
    {
      uint64_t v7 = -[MCAsset imprint](v6, "imprint");
      objc_super v8 = @"asset";
    }

    else
    {
      uint64_t v7 = -[MCObject objectID](v6, "objectID");
      objc_super v8 = @"assetID";
    }
...
- (void)setAsset:(id)a3
{
  if (self->mAsset != a3)
  {
    ...
    if (mAsset)
    {
      -[MCAssetAudio removeSong:](mAsset, "removeSong:", self);
    }
    ...
    if (a3)
    {
      v6 = (MCAssetAudio *)a3;
      self->mAsset = v6;
      -[MCAssetAudio addSong:](v6, "addSong:", self);
    }
...
- (void)_copySelfToSnapshot:(id)a3
{
  *((void *)a3 + 4) = -[MCObject snapshot](self->mAsset, "snapshot");
  ...
  *((_BYTE *)a3 + 24) = self->mStartTimeIsDefined;
  *((void *)a3 + 9) = *(void *)&self->mDuration;
  *((_BYTE *)a3 + 25) = self->mDurationIsDefined;
  objc_sync_exit(self);
}
