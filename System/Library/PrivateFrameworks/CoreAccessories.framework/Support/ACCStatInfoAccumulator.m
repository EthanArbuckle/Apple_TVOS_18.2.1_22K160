...
{
  v6.receiver = self;
  v6.super_class = (Class)&OBJC_CLASS___ACCStatInfoAccumulator;
  v2 = -[ACCStatInfoAccumulator init](&v6, "init");
  v3 = v2;
  if (v2)
  {
    name = v2->_name;
...
- (ACCStatInfoAccumulator)initWithName:(id)a3
{
  id v5 = a3;
  v9.receiver = self;
  v9.super_class = (Class)&OBJC_CLASS___ACCStatInfoAccumulator;
  objc_super v6 = -[ACCStatInfoAccumulator init](&v9, "init");
  v7 = v6;
  if (v6)
  {
...
- (ACCStatInfoAccumulator)initWithName:(id)a3 initValue:(unint64_t)a4
{
  id v7 = a3;
  v11.receiver = self;
  v11.super_class = (Class)&OBJC_CLASS___ACCStatInfoAccumulator;
  v8 = -[ACCStatInfoAccumulator init](&v11, "init");
  objc_super v9 = v8;
  if (v8)
  {
...
- (id)description
{
  ...
  return +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"[ACCStatInfoAccumulator:%@ start %llu, max %llu, last %llu, accum %llu, count %llu, average %llu, startTime=%@, lastUpdateTime=%@]",  name,  self->_start,  self->_max,  self->_last,  self->_accum,  self->_count,  self->_average,  self->_startTime,  self->_lastUpdateTime);
}
...
{
  id v4 = a3;
  id v5 = (NSDate *)objc_claimAutoreleasedReturnValue([v4 startTime]);
  startTime = self->_startTime;
  self->_startTime = v5;

  id v7 = (NSDate *)objc_claimAutoreleasedReturnValue([v4 lastUpdateTime]);
  lastUpdateTime = self->_lastUpdateTime;
