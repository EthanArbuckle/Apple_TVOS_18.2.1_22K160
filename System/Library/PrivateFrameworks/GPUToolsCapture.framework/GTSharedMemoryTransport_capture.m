...
- (GTSharedMemoryTransport_capture)initWithMode:(int)a3
{
  return -[GTSharedMemoryTransport_capture initWithMode:initialMessageSerial:]( self,  "initWithMode:initialMessageSerial:",  *(void *)&a3,  0LL);
}

- (GTSharedMemoryTransport_capture)initWithMode:(int)a3 initialMessageSerial:(unsigned int)a4
{
  v14.receiver = self;
  v14.super_class = (Class)&OBJC_CLASS___GTSharedMemoryTransport_capture;
  v6 = -[GTBaseStreamTransport_capture init](&v14, "init");
  v7 = v6;
  if (!v6) {
    return v7;
  }
  ...
  id v11 = -[GTSharedMemoryTransport_capture methodForSelector:](v7, "methodForSelector:");
...
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___GTSharedMemoryTransport_capture;
  -[GTBaseStreamTransport_capture dealloc](&v3, "dealloc");
}
...
- (void)setUrl:(id)a3
{
  ...
  if (-[GTSharedMemoryTransport_capture connected](self, "connected"))
  {
    if (s_logUsingOsLog)
    {
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5 replyQueue:(id)a6 timeout:(unint64_t)a7 handler:(id)a8
{
  if (self->_deferred)
  {
    v13 = objc_alloc_init(&OBJC_CLASS___GTBufferedSendOperation_capture);
    -[GTBufferedSendOperation_capture setMessage:](v13, "setMessage:", a3);
    -[GTBufferedSendOperation_capture setReplyTo:](v13, "setReplyTo:", a4);
    -[GTBufferedSendOperation_capture setQueue:](v13, "setQueue:", a6);
    -[GTBufferedSendOperation_capture setReplyBlock:](v13, "setReplyBlock:", a8);
  ...
  {
    v15.receiver = self;
    v15.super_class = (Class)&OBJC_CLASS___GTSharedMemoryTransport_capture;
    return -[GTTransport_capture send:inReplyTo:error:replyQueue:timeout:handler:]( &v15,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  a4,  a5,  a6,  a7,  a8);
  }
...
- (void)_mapSharedMemoryFile:(int)a3 size:(unint64_t)a4 error:(id *)a5
{
  size_t v8 = -[GTSharedMemoryTransport_capture _calculateVMRegionMapSize:](self, "_calculateVMRegionMapSize:", a4);
...
- (void)_tearDownSharedMemory
{
  ...
  if (incomingShmem)
  {
    munmap( incomingShmem,  -[GTSharedMemoryTransport_capture _calculateVMRegionMapSize:]( self,  "_calculateVMRegionMapSize:",  incomingShmem[6]));
  ...
  if (outgoingShmem)
  {
    munmap( outgoingShmem,  -[GTSharedMemoryTransport_capture _calculateVMRegionMapSize:]( self,  "_calculateVMRegionMapSize:",  outgoingShmem[6]));
...
- (BOOL)_initializeSMRegion:(sm_region_header *)a3 size:(unint64_t)a4 name:(const char *)a5 error:(id *)a6
{
  ...
  {
    v12 = -[GTSharedMemoryTransport_capture _mapSharedMemoryFile:size:error:]( self,  "_mapSharedMemoryFile:size:error:",  v11,  v9,  a6);
    if (v12)
    {
      v13 = v12;
      close(v11);
...
- (BOOL)_openSMRegion:(sm_region_header *)a3 size:(unint64_t)a4 name:(const char *)a5 error:(id *)a6
{
  ...
  id v16 = -[GTSharedMemoryTransport_capture _mapSharedMemoryFile:size:error:]( self,  "_mapSharedMemoryFile:size:error:",  v12,  v15,  a6);
  if (v16)
  {
    v17 = v16;
    close(v14);
...
- (BOOL)_createAndRunSources:(id *)a3
{
  ...
  uint64_t v15 = 0LL;
  v4 = objc_alloc(&OBJC_CLASS___GTContinuation_capture);
  ...
  v9[4] = self;
  v9[5] = &v10;
  v6 = -[GTContinuation_capture initWithQueue:block:](v4, "initWithQueue:block:", queue, v9);
  v11[5] = (uint64_t)v6;
...
- (BOOL)_serverConnect:(id *)a3
{
  if (!-[GTSharedMemoryTransport_capture _initializeSMRegion:size:name:error:]( self,  "_initializeSMRegion:size:name:error:",  &self->_outgoingShmem,  self->_sm_region_size_small,  self->_sendName,  a3)
    || !-[GTSharedMemoryTransport_capture _initializeSMRegion:size:name:error:]( self,  "_initializeSMRegion:size:name:error:",  &self->_incomingShmem,  self->_sm_region_size_large,  self->_receiveName,  a3))
  {
    return 0;
  }

  -[GTSharedMemoryTransport_capture _setupIOBuffers](self, "_setupIOBuffers");
  BOOL result = -[GTSharedMemoryTransport_capture _createAndRunSources:](self, "_createAndRunSources:", a3);
...
- (BOOL)_clientConnect:(id *)a3
{
  if (!-[GTSharedMemoryTransport_capture _openSMRegion:size:name:error:]( self,  "_openSMRegion:size:name:error:",  &self->_incomingShmem,  self->_sm_region_size_small,  self->_receiveName,  a3)
    || !-[GTSharedMemoryTransport_capture _openSMRegion:size:name:error:]( self,  "_openSMRegion:size:name:error:",  &self->_outgoingShmem,  self->_sm_region_size_large,  self->_sendName,  a3))
  {
    return 0;
  }
...
- (id)connect
{
  id v3 = +[GTFuture_capture future](&OBJC_CLASS___GTFuture_capture, "future");
  queue = (dispatch_queue_s *)self->super.super._queue;
...
- (void)_invalidate
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___GTSharedMemoryTransport_capture;
  -[GTBaseStreamTransport_capture _invalidate](&v3, "_invalidate");
}
...
- (void)_waitEAGAIN
{
  ...
  {
    usleep(0xC8u);
    if (-[GTSharedMemoryTransport_capture _computeBytesAvailableToRead](self, "_computeBytesAvailableToRead") > 4) {
      break;
    }
  }
...
- (int64_t)_write:(const void *)a3 size:(unint64_t)a4
{
  ...
  if (a4)
  {
    ...
    unint64_t v8 = -[GTSharedMemoryTransport_capture _computeBytesAvailableToWrite](self, "_computeBytesAvailableToWrite");
    if (v8 > 4)
    {
      if (v8 - 4 < v4) {
        unint64_t v4 = v8 - 4;
      ...
      int v12 = v4 + 4;
      -[GTSharedMemoryTransport_capture _copyToSM:size:](self, "_copyToSM:size:", &v12, 4LL);
      -[GTSharedMemoryTransport_capture _copyToSM:size:](self, "_copyToSM:size:", a3, v4);
...
- (void)_copyFromSM:(void *)a3 size:(unint64_t)a4
{
  ...
  -[GTSharedMemoryTransport_capture _updateReaderOffset:](self, "_updateReaderOffset:", v6 + a4);
}
...
- (int64_t)_read:(void *)a3 size:(unint64_t)a4
{
  ...
  if ((_DWORD)currentPacketBytesLeft)
  {
    ...
    -[GTSharedMemoryTransport_capture _copyFromSM:size:](self, "_copyFromSM:size:", a3, v6);
...
- (void)_accumulateMessageBytes
{
  while (self->_currentPacketBytesLeft
       && -[GTBaseStreamTransport_capture _readAndAccumulate](self, "_readAndAccumulate") > 0)
    ;
}
...
- (void)_relayPacket
{
  ...
  -[GTBaseStreamTransport_capture _relayBuffer:size:]( self->_relayTransport,  "_relayBuffer:size:",  &incomingBuffer[v4],  self->_currentPacketBytesLeft);
  unsigned int v5 = atomic_load((unsigned int *)self->_incomingShmem + 5);
  -[GTSharedMemoryTransport_capture _updateReaderOffset:]( self,  "_updateReaderOffset:",  self->_currentPacketBytesLeft + v5);
  self->_unint64_t currentPacketBytesLeft = 0;
}

- (void)_dequeuePacket
{
  unint64_t v3 = -[GTSharedMemoryTransport_capture _computeBytesAvailableToRead](self, "_computeBytesAvailableToRead");
  if (v3 <= 3)
  {
    unsigned int v5 = objc_autoreleasePoolPush();
    -[GTBaseStreamTransport_capture _scheduleInvalidation:]( self,  "_scheduleInvalidation:",  +[GTError_capture errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_capture,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  259LL,  0LL));
  ...
  {
    unint64_t v4 = v3;
    -[GTSharedMemoryTransport_capture _copyFromSM:size:](self, "_copyFromSM:size:", &v7, 4LL);
    if (v4 < v7)
    {
...
- (void)_dequeueIncomingMessages
{
  ...
  while (1)
  {
    while (1)
    {
      ...
      -[GTSharedMemoryTransport_capture _dequeuePacket](self, "_dequeuePacket");
      if (!self->_currentPacketBytesLeft) {
        return;
      }
      ((void (*)(GTSharedMemoryTransport_capture *, SEL))self->_consumeBytesIMP)( self,  self->_consumeBytesSEL);
      if (self->_currentPacketBytesLeft) {
        goto LABEL_23;
      }
