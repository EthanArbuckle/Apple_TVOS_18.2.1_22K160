...
{
  v9.receiver = self;
  v9.super_class = (Class)&OBJC_CLASS___GTBaseStreamTransport_tools_diagnostics;
  v2 = -[GTTransport_tools_diagnostics init](&v9, "init");
  v3 = v2;
  if (v2)
  {
    ...
    if (v4)
    {
      *(void *)(v4 + 24) = 0x1000000001LL;
      *(_OWORD *)(v4 + 32) = xmmword_197C0;
      *(_BYTE *)(v4 + 48) = 0;
      vm_address_t v5 = GTCoreAlloc(v4, 0x31uLL);
      v3->_relayTmu = (dy_transport_message_unpack_s *)v5;
      if (v5)
      {
        *(void *)(v5 + 24) = 0x100000001LL;
        *(_OWORD *)(v5 + 32) = xmmword_197C0;
        *(_BYTE *)(v5 + 48) = 0;
        v6 = -[NSString UTF8String]( +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"gputools.%@.%p.%@",  +[NSString stringWithUTF8String:]( &OBJC_CLASS___NSString,  "stringWithUTF8String:",  object_getClassName(v3)),  v3,  @"relay"),  "UTF8String");
        v3->_relayQueue = (OS_dispatch_queue *)dispatch_queue_create_with_target_V2( v6,  0LL,  (dispatch_queue_t)v3->super._rootQueue);
        v3->_sendSema = (OS_dispatch_semaphore *)dispatch_semaphore_create(1LL);
        return v3;
      }
...
- (void)dealloc
{
  ...
  v5.receiver = self;
  v5.super_class = (Class)&OBJC_CLASS___GTBaseStreamTransport_tools_diagnostics;
  -[GTTransport_tools_diagnostics dealloc](&v5, "dealloc");
}
...
- (void)_invalidate
{
  ...
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___GTBaseStreamTransport_tools_diagnostics;
  -[GTTransport_tools_diagnostics _invalidate](&v3, "_invalidate");
}

- (void)_scheduleInvalidation:(id)a3
{
  v4.receiver = self;
  v4.super_class = (Class)&OBJC_CLASS___GTBaseStreamTransport_tools_diagnostics;
  -[GTTransport_tools_diagnostics _scheduleInvalidation:](&v4, "_scheduleInvalidation:", a3);
...
- (int64_t)_syncTmuToHeader:(dy_transport_message_unpack_s *)a3 ioBlock:(id)a4
{
  ...
  if (result != -1 && !a3->var3)
  {
    ...
    *(int8x16_t *)&a3->var0.var0 = vrev32q_s8(*(int8x16_t *)&a3->var0.var0);
    ...
    -[GTBaseStreamTransport_tools_diagnostics _scheduleInvalidation:]( self,  "_scheduleInvalidation:",  +[GTError_tools_diagnostics errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_tools_diagnostics,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  v11,  0LL));
...
- (void)_unpackAndDispatchMessage
{
  ...
  if ((tmu->var0.var4 & 0x80000000) != 0)
  {
    ...
    if (!v10)
    {
      -[GTBaseStreamTransport_tools_diagnostics _scheduleInvalidation:]( self,  "_scheduleInvalidation:",  +[GTError_tools_diagnostics errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_tools_diagnostics,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  260LL,  0LL));
  ...
  v14 = -[GTTransportMessage_tools_diagnostics initWithKind:attributes:payload:]( objc_alloc(&OBJC_CLASS___GTTransportMessage_tools_diagnostics),  "initWithKind:attributes:payload:",  tmu->var0.var1,  v8,  v12);
  -[GTTransportMessage_tools_diagnostics _setSerial:replySerial:transport:]( v14,  "_setSerial:replySerial:transport:",  self->_tmu->var0.var2,  self->_tmu->var0.var3,  self);
  -[GTTransportMessage_tools_diagnostics _setTransportSize:](v14, "_setTransportSize:", v9);
  objc_autoreleasePoolPop(v3);
  messageBuffer = self->_messageBuffer;
  uint64_t v17 = objc_opt_class(&OBJC_CLASS___GTVMBuffer_tools_diagnostics, v16);
  ...
  self->_messageBuffer = 0LL;
  -[GTTransport_tools_diagnostics _dispatchMessage:](self, "_dispatchMessage:", v14);

  v19 = self->_tmu;
  *(_OWORD *)&v19->var3 = xmmword_197C0;
...
- (int64_t)_performRead:(void *)a3 size:(unint64_t)a4
{
  ...
  do
  {
    int64_t result = -[GTBaseStreamTransport_tools_diagnostics _read:size:](self, "_read:size:", a3, self->_bytesToRead);
    if (result != -1)
    {
      if (result)
      {
      ...
      else
      {
        ...
        -[GTBaseStreamTransport_tools_diagnostics _scheduleInvalidation:]( self,  "_scheduleInvalidation:",  +[GTError_tools_diagnostics errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_tools_diagnostics,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  31LL,  0LL));
...
- (int64_t)_readAndAccumulate
{
  ...
  else
  {
    ...
    {
LABEL_16:
      -[GTBaseStreamTransport_tools_diagnostics _unpackAndDispatchMessage](self, "_unpackAndDispatchMessage");
      return v4;
    }

    -[GTBaseStreamTransport_tools_diagnostics _allocateMessageBuffer](self, "_allocateMessageBuffer");
  ...
  int64_t v11 = -[GTBaseStreamTransport_tools_diagnostics _performRead:size:]( self,  "_performRead:size:",  self->_messageBufferWritePtr,  var3);
  if (v11 == -1) {
    return -1LL;
  }
  v4 += v11;
  ...
  if (v6)
  {
    ...
    while (1)
    {
      do
      {
        while (1)
        {
          unint64_t v15 = -[GTBaseStreamTransport_tools_diagnostics _write:size:]( self,  "_write:size:",  iov->iov_base,  iov->iov_len);
...
- (BOOL)_packMessage:(id)a3 error:(id *)a4
{
  if ([a3 attributes])
  {
    ...
    if (!v7)
    {
      if (a4)
      {
        ...
        *a4 = +[GTError_tools_diagnostics errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_tools_diagnostics,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  36LL,  v13);
      }
  ...
  if (a4)
  {
    id v12 = +[GTError_tools_diagnostics errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_tools_diagnostics,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  261LL,  0LL);
    BOOL result = 0;
    *a4 = v12;
    return result;
  }
...
LABEL_10:
  int64_t v15 = -[GTBaseStreamTransport_tools_diagnostics _writeBuffers:](self, "_writeBuffers:", a4);
  dispatch_semaphore_signal((dispatch_semaphore_t)self->_sendSema);
  return v15;
}
...
LABEL_5:
  int64_t v8 = -[GTBaseStreamTransport_tools_diagnostics _writeBuffers:](self, "_writeBuffers:", 0LL);
  int64_t v9 = v8;
  if (v8 == -1)
  {
LABEL_8:
...
LABEL_9:
  ...
  if (!v11)
  {
    *(_OWORD *)&v10->unint64_t var3 = xmmword_197C0;
...
- (int64_t)_relayBuffer:(const void *)a3 size:(unint64_t)a4
{
  ...
  block[7] = a4;
  char v12 = v7;
  dispatch_sync((dispatch_queue_t)relayQueue, block);
  int64_t v9 = v14[3];
