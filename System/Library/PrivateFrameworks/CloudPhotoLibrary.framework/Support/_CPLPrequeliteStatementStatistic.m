...
- (_CPLPrequeliteStatementStatistic)initWithStatement:(id)a3
{
  id v4 = a3;
  v9.receiver = self;
  v9.super_class = (Class)&OBJC_CLASS____CPLPrequeliteStatementStatistic;
  v5 = -[_CPLPrequeliteStatementStatistic init](&v9, "init");
  if (v5)
  {
    v6 = (NSString *)[v4 copy];
...
- (id)description
{
  v3 = -[NSMutableString initWithFormat:]( objc_alloc(&OBJC_CLASS___NSMutableString),  "initWithFormat:",  @"%lu * %@",  self->_count,  self->_statement);
  unint64_t v4 = v3;
  ...
  if (durationInNanoSeconds)
  {
    ...
    if (resultsCount) {
      -[NSMutableString appendFormat:]( v3,  "appendFormat:",  @" (%lu results - %0.0fms)",  resultsCount,  *(void *)&v7);
    }
    else {
      -[NSMutableString appendFormat:](v3, "appendFormat:", @" (%0.0fms)", *(void *)&v7, v9);
    }
  }

  else if (resultsCount)
  {
    -[NSMutableString appendFormat:](v3, "appendFormat:", @" (%lu results)", self->_resultsCount, v9);
  }
