...
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___BLEMIDITimeStamper;
  result = -[BLEMIDITimeStamper init](&v3, "init");
  if (result) {
...
- (void)reset
{
  ...
  self->lastGeneratedTimestamp = 0LL;
  self->lastReceivedOffsetTimestamp = 0LL;
  self->lastReceivedTime = 0LL;
  self->hasBeenReset = 1;
}
...
- (unint64_t)generateTimeStampForOffset:(unsigned __int16)a3
{
  ...
  if (lastReceivedTime)
  {
    if (-[BLEMIDITimeStamper offsetIsUsable:numberOfWraps:](self, "offsetIsUsable:numberOfWraps:", a3, &v22))
    {
    ...
    else
    {
      ...
      -[BLEMIDITimeStamper reset](self, "reset");
...
- (BOOL)offsetIsUsable:(unsigned __int16)a3 numberOfWraps:(unsigned __int16 *)a4
{
  ...
  if (v9 > -4097)
  {
    int v16 = -[BLEMIDITimeStamper modsignWithTimeDifference:range:]( self,  "modsignWithTimeDifference:range:",  v4 - v8,  0x2000LL);
    if (v16 >= 0) {
