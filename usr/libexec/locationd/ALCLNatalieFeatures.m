...
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___ALCLNatalieFeatures;
  return +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"%@ %@",  -[ALCLNatalieFeatures description](&v3, "description"),  -[ALCLNatalieFeatures dictionaryRepresentation](self, "dictionaryRepresentation"));
}
...
- (id)dictionaryRepresentation
{
  ...
  *(float *)&double v4 = self->_fVectorMagnitude;
  objc_msgSend( v3,  "setObject:forKey:",  +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v4),  @"fVectorMagnitude");
  *(float *)&double v5 = self->_fIQRX;
  objc_msgSend( v3,  "setObject:forKey:",  +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v5),  @"fIQRX");
  *(float *)&double v6 = self->_fIQRY;
  objc_msgSend( v3,  "setObject:forKey:",  +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v6),  @"fIQRY");
  *(float *)&double v7 = self->_fIQRZ;
  objc_msgSend( v3,  "setObject:forKey:",  +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v7),  @"fIQRZ");
  *(float *)&double v8 = self->_fMachineFrequency;
  objc_msgSend( v3,  "setObject:forKey:",  +[NSNumber numberWithFloat:](NSNumber, "numberWithFloat:", v8),  @"fMachineFrequency");
  uint64_t fEpochType = self->_fEpochType;
  if (fEpochType >= 3) {
    v10 = +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"(unknown: %i)",  self->_fEpochType);
  }
  else {
    v10 = (NSString *)*(&off_10182C988 + fEpochType);
  }
  [v3 setObject:v10 forKey:@"fEpochType"];
  objc_msgSend( v3,  "setObject:forKey:",  +[NSNumber numberWithBool:](NSNumber, "numberWithBool:", self->_fStanding),  @"fStanding");
  return v3;
}

- (BOOL)readFrom:(id)a3
{
  return sub_100253FF0((uint64_t)self, (uint64_t)a3);
}
...
- (unint64_t)hash
{
  ...
  return v20 ^ v11 ^ v29 ^ v38 ^ v46 ^ (2654435761LL * self->_fEpochType) ^ (2654435761LL * self->_fStanding);
}
