...
- (GKDelayedSetPlayerStatusCacheReader)initWithDatabaseConnection:(id)a3
{
  id v4 = a3;
  v8.receiver = self;
  v8.super_class = (Class)&OBJC_CLASS___GKDelayedSetPlayerStatusCacheReader;
  id v5 = -[GKDelayedSetPlayerStatusCacheReader init](&v8, "init");
  v6 = v5;
  if (v5) {
...
- (id)setStatusDescriptorStatement
{
  id v2 = sub_1000AD0A4(off_100270808);
  v3 = (void *)objc_claimAutoreleasedReturnValue(v2);
  id v4 = (void *)objc_claimAutoreleasedReturnValue( +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"    SELECT %@     FROM requests         INNER JOIN statuses USING (request_id)     WHERE player_gcid = %s     ORDER BY time_stamp DESC;",
                   v3,
                   ":player_gcid"));
...
- (void)readResources:(id)a3 handler:(id)a4
{
  ...
  {
    v18 = +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"%@ is being asked to read statuses for multiple players. This is not currently supported.",  objc_opt_class(self, v8));
  ...
  {
    v9 = (void *)objc_claimAutoreleasedReturnValue( +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"%s:%d %s",  "GKDelayedSetPlayerStatusCacheReader.mm",  82LL,  "-[GKDelayedSetPlayerStatusCacheReader readResources:handler:]"));
