...
- (void)dealloc
{
  ...
  {
    v5 = [NSString stringWithFormat:@"must invoke -invalidate before deallocating BKCADisplayMonitor"];
    if (os_log_type_enabled((os_log_t)&_os_log_default, OS_LOG_TYPE_ERROR))
    {
      v6 = NSStringFromSelector(a2);
      v7 = (void *)[v6 autorelease];
      v9 = [self class];
  ...
  v12.receiver = self;
  [v12 super_class] = [BKCADisplayMonitor class];
  [v12 dealloc];
}

- (BKCADisplayMonitor)init
{
  BKSharedCADisplayProvider *v3 = [[BKSharedCADisplayProvider sharedInstance] autorelease];
  BKCADisplayMonitor *v4 = [[BKCADisplayMonitor alloc] initWithDisplayProvider:v3 log:0LL filterPredicate:0LL];
...
- (BKCADisplayMonitor)initWithLog:(id)a3
{
  ...
  BKCADisplayMonitor *v6 = [BKCADisplayMonitor initWithDisplayProvider:v5 log:v4 filterPredicate:0LL];
...
- (BKCADisplayMonitor)initWithLog:(id)a3 filterPredicate:(id)a4
{
  id v6 = a4;
  id v7 = a3;
  uint64_t v8 = (void *)[BKSharedCADisplayProvider sharedInstance];
  BKCADisplayMonitor *v9 = [[BKCADisplayMonitor alloc] initWithDisplayProvider:v8 log:v7 filterPredicate:v6];
...
- (BKCADisplayMonitor)initWithDisplayProvider:(id)a3 log:(id)a4 filterPredicate:(id)a5
{
  ...
  id v10 = a5;
  v29.receiver = self;
  v29.super_class = [BKCADisplayMonitor class];
  BKCADisplayMonitor *v11 = [[BKCADisplayMonitor alloc] init];
  if (v11)
  {
    if (!v8) {
      BKSharedCADisplayProvider *v8 = [[BKSharedCADisplayProvider sharedInstance] autorelease];
    }
    v11->_displayProvider = v8;
    if (v9)
    {
    ...
    __int16 v19 = [v11 _immutableDisplays];
    uint64_t v20 = [v11 _filterDisplays:v19 usingPredicate:v11->_filterPredicateLock_displayFilterPredicate];
    ...
    [v11->_displayProvider addMonitor:v23];
  }
...
- (void)invalidate
{
  ...
  if (!self->_displayLock_isInvalid)
  {
    ...
    [self->_displayProvider removeMonitor:self];
    displayProvider = self->_displayProvider;
    self->_displayProvider = 0LL;

    [self->_displayLock_observerAssertion invalidate];
...
- (id)immutableDisplayForUUID:(id)a3
{
  ...
  v11 = [displayLock_displayUUIDKeyToImmutableCADisplay objectForKeyedSubscript:v10];
  os_unfair_lock_unlock(p_displayLock);
...
- (id)addObserver:(id)a3
{
  ...
  {
    BSCompoundAssertion *v8 = [BSCompoundAssertion assertionWithIdentifier:@"backboardd.CADisplayMonitor"];
  ...
  [v5 description];
  v11 = (void *)v10;
  BSCompoundAssertion *v12 = [BSCompoundAssertion acquireForReason:v11 withContext:v5];
...
- (BOOL)displayUUIDIsActive:(id)a3
{
  ...
  id v5 = a3;
  os_unfair_lock_lock(p_displayLock);
  uint64_t v6 = [self->_displayLock_displayUUIDKeyToImmutableCADisplay objectForKey:v5];
...
- (NSArray)activeDisplays
{
  p_displayLock = &self->_displayLock;
  os_unfair_lock_lock(&self->_displayLock);
  NSArray *v4 = [self->_displayLock_displayUUIDKeyToImmutableCADisplay allValues];
  os_unfair_lock_unlock(p_displayLock);
  return (NSArray *)v4;
}
...
- (BOOL)hasActiveDisplays
{
  ...
  p_displayLock = &self->_displayLock;
  os_unfair_lock_lock(&self->_displayLock);
  BOOL v2 = [v2->_displayLock_displayUUIDKeyToImmutableCADisplay count] != 0LL;
  os_unfair_lock_unlock(p_displayLock);
  return (char)v2;
}
...
- (id)_immutableDisplays
{
  v3 = [BKCADisplayProvider displays];
  id v4 = [v3 bs_map:&stru_1000B7AA8];
...
- (id)_filterDisplays:(id)a3 usingPredicate:(id)a4
{
  id v6 = a3;
  id v7 = a4;
  NSMutableDictionary *v19 = [[NSMutableDictionary alloc] init];
  __int128 v20 = 0u;
  __int128 v21 = 0u;
  __int128 v22 = 0u;
  __int128 v23 = 0u;
...
- (void)setDisplayFilterPredicate:(id)a3
{
  ...
  os_unfair_lock_unlock(&self->_filterPredicateLock);
  [self reevaluateActiveDisplaysWithReason:@"predicate changed"];
}
...
- (void)reevaluateActiveDisplaysWithReason:(id)a3
{
  ...
  os_unfair_lock_unlock(&self->_filterPredicateLock);
  v39 = [BKCADisplayMonitor _immutableDisplays];
  v40 = v6;
  id v7 = [self _filterDisplays:usingPredicate:];
  os_unfair_lock_lock(&self->_displayLock);
  NSMutableDictionary *v8 = [self->_displayLock_displayUUIDKeyToImmutableCADisplay allValues];
  id v9 = [NSSet setWithArray:v8];

  int v10 = (void *)[v7 allValues];
  id v11 = [NSSet setWithArray:v10];

  v38 = v7;
  self->_displayLock_displayUUIDKeyToImmutableCADisplay = v7;
  [self->_displayLock_displayIndexToDisplayUUID removeAllObjects];
  displayLock_displayUUIDKeyToImmutableCADisplay = self->_displayLock_displayUUIDKeyToImmutableCADisplay;
  ...
  v61[3] = &unk_1000B7A68;
  v61[4] = self;
  [displayLock_displayUUIDKeyToImmutableCADisplay enumerateKeysAndObjectsUsingBlock:v61];
  id v13 = [self->_displayLock_observerAssertion context];
  id v14 = [v13 copy];
        ...
        if (v27)
        {
          ...
          do
          {
            for (k = 0LL; k != v28; k = (char *)k + 1)
            {
              ...
              [v20 monitor:self displayDidBecomeInactive:k];
            }
