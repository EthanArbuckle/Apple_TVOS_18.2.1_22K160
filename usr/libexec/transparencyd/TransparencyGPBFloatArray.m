...
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___TransparencyGPBFloatArray;
  return -[TransparencyGPBFloatArray init](&v3, "init");
}

- (TransparencyGPBFloatArray)initWithValueArray:(id)a3
{
  return -[TransparencyGPBFloatArray initWithValues:count:]( self,  "initWithValues:count:",  *((void *)a3 + 2),  *((void *)a3 + 3));
}
...
- (TransparencyGPBFloatArray)initWithCapacity:(unint64_t)a3
{
  float v4 = -[TransparencyGPBFloatArray initWithValues:count:](self, "initWithValues:count:", 0LL, 0LL);
  v5 = v4;
  if (a3 && v4) {
...
- (id)copyWithZone:(_NSZone *)a3
{
  return -[TransparencyGPBFloatArray initWithValues:count:]( +[TransparencyGPBFloatArray allocWithZone:](&OBJC_CLASS___TransparencyGPBFloatArray, "allocWithZone:", a3),  "initWithValues:count:",  self->_values,  self->_count);
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  uint64_t v5 = objc_opt_class(&OBJC_CLASS___TransparencyGPBFloatArray, a2);
...
{
  float v3 = a3;
  -[TransparencyGPBFloatArray addValues:count:](self, "addValues:count:", &v3, 1LL);
}
...
- (void)addValues:(const float *)a3 count:(unint64_t)a4
{
  if (a3 && a4)
  {
    ...
    if (count + a4 > self->_capacity) {
      -[TransparencyGPBFloatArray internalResizeToCapacity:]( self,  "internalResizeToCapacity:",  (v8 & 0xFFFFFFFFFFFFFFF0LL) + 16);
    }
...
- (void)removeValueAtIndex:(unint64_t)a3
{
  ...
  if (count + 31 < self->_capacity) {
    -[TransparencyGPBFloatArray internalResizeToCapacity:]( self,  "internalResizeToCapacity:",  (v6 & 0xFFFFFFFFFFFFFFF0LL) + 16);
  }
}
...
- (void)removeAll
{
  ...
  if (self->_capacity >= 0x21) {
    -[TransparencyGPBFloatArray internalResizeToCapacity:](self, "internalResizeToCapacity:", 16LL);
  }
}
