...
- (TransparencyGPBBoolFloatDictionary)init
{
  return -[TransparencyGPBBoolFloatDictionary initWithFloats:forKeys:count:]( self,  "initWithFloats:forKeys:count:",  0LL,  0LL,  0LL);
}

- (TransparencyGPBBoolFloatDictionary)initWithFloats:(const float *)a3 forKeys:(const BOOL *)a4 count:(unint64_t)a5
{
  v13.receiver = self;
  v13.super_class = (Class)&OBJC_CLASS___TransparencyGPBBoolFloatDictionary;
...
- (TransparencyGPBBoolFloatDictionary)initWithDictionary:(id)a3
{
  result = -[TransparencyGPBBoolFloatDictionary initWithFloats:forKeys:count:]( self,  "initWithFloats:forKeys:count:",  0LL,  0LL,  0LL);
  if (a3 && result)
  {
    uint64_t v5 = 0LL;
    char v6 = 1;
...
- (id)copyWithZone:(_NSZone *)a3
{
  return -[TransparencyGPBBoolFloatDictionary initWithDictionary:]( +[TransparencyGPBBoolFloatDictionary allocWithZone:]( &OBJC_CLASS___TransparencyGPBBoolFloatDictionary,  "allocWithZone:",  a3),  "initWithDictionary:",  self);
}
...
- (BOOL)isEqual:(id)a3
{
  ...
  uint64_t v5 = objc_opt_class(&OBJC_CLASS___TransparencyGPBBoolFloatDictionary, a2);
...
- (id)description
{
  ...
  v4 = v3;
  if (self->_valueSet[0]) {
    -[NSMutableString appendFormat:](v3, "appendFormat:", @"NO: %f", self->_values[0]);
  }
  if (self->_valueSet[1]) {
    -[NSMutableString appendFormat:](v4, "appendFormat:", @"YES: %f", self->_values[1]);
  }
...
{
  if (self->_valueSet[0]) {
    (*((void (**)(id, const __CFString *, NSString *))a3 + 2))( a3,  @"false",  +[NSString stringWithFormat:](&OBJC_CLASS___NSString, "stringWithFormat:", @"%.*g", 6LL, self->_values[0]));
  }
  if (self->_valueSet[1]) {
    (*((void (**)(id, const __CFString *, NSString *))a3 + 2))( a3,  @"true",  +[NSString stringWithFormat:](&OBJC_CLASS___NSString, "stringWithFormat:", @"%.*g", 6LL, self->_values[1]));
  }
}
...
- (void)enumerateKeysAndFloatsUsingBlock:(id)a3
{
  char v5 = 0;
  if (!self->_valueSet[0]
    || ((*((void (**)(id, void, char *, float))a3 + 2))(a3, 0LL, &v5, self->_values[0]), !v5))
  {
    if (self->_valueSet[1]) {
      (*((void (**)(id, uint64_t, char *, float))a3 + 2))(a3, 1LL, &v5, self->_values[1]);
    }
  }
...
- (unint64_t)computeSerializedSizeAsField:(id)a3
{
  ...
  do
  {
    ...
    if (valueSet[v3])
    {
      ...
      uint64_t v11 = sub_10010F960(2) + v10;
...
- (void)writeToCodedOutputStream:(id)a3 asField:(id)a4
{
  ...
  do
  {
    ...
    if (valueSet[v8])
    {
      ...
      [a3 writeInt32NoTag:sub_10010F960(2) + v14];
