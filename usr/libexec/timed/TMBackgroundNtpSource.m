...
- (void)setFetchingTime:(BOOL)a3
{
  ...
  if (fetchingTime == a3)
  {
    ...
    if (os_log_type_enabled((os_log_t)qword_1000338F8, OS_LOG_TYPE_ERROR)) {
      sub_10001BC60(fetchingTime, v3, v6);
    }
  }
...
{
  v4.receiver = self;
  v4.super_class = (Class)&OBJC_CLASS___TMBackgroundNtpSource;
  v2 = -[TMBackgroundNtpSource init](&v4, "init");
  if (v2) {
    v2->_ntpQueue = (OS_dispatch_queue *)dispatch_queue_create("NTP Queue", 0LL);
  }
  return v2;
}
...
- (void)dealloc
{
  ...
  -[TMBackgroundNtpSource setReachabilityTransaction:](self, "setReachabilityTransaction:", 0LL);
...
- (void)setWantedTime:(double)a3
{
  ...
  if (Current >= a3 && v6 == 0.0)
  {
    ...
    block[2] = sub_100014558;
    block[3] = &unk_100028DB0;
    block[4] = self;
    dispatch_async((dispatch_queue_t)ntpQueue, block);
    return;
  }

  -[TMMonotonicClock coarseMonotonicTime](-[TMBackgroundNtpSource clock](self, "clock"), "coarseMonotonicTime");
  double v10 = v9;
  if (Current + v6 <= a3) {
    double v11 = a3;
  }
...
- (OS_os_transaction)reachabilityTransaction
{
  return (OS_os_transaction *)objc_getProperty(self, a2, 104LL, 1);
}
