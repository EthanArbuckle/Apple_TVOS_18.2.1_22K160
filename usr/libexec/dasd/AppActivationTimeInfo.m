...
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___AppActivationTimeInfo;
  result = -[AppActivationTimeInfo init](&v3, "init");
  if (result)
  {
    *(void *)&result->_launchCount = 0LL;
...
- (id)description
{
  return +[NSString stringWithFormat:]( &OBJC_CLASS___NSString,  "stringWithFormat:",  @"Cold Launches: %d -- Total Launch Activation Time: %f ***** Resumes: %d - Total Resume Activation Time %f",  self->_launchCount,  *(void *)&self->_totalLaunchActivationTime,  self->_resumeCount,  *(void *)&self->_totalResumeActivationTime);
}
