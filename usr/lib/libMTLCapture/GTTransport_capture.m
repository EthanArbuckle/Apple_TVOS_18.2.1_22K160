...
{
  v8.receiver = self;
  v8.super_class = (Class)&OBJC_CLASS___GTTransport_capture;
  id v2 = -[GTTransport_capture init](&v8, "init");
  if (v2)
  {
    ...
    *((void *)v2 + 9) = objc_opt_new(&OBJC_CLASS___GTIntKeyedDictionary_capture);
...
- (void)dealloc
{
  ...
  v6.receiver = self;
  v6.super_class = (Class)&OBJC_CLASS___GTTransport_capture;
  -[GTTransport_capture dealloc](&v6, "dealloc");
}
...
- (id)debugDescription
{
  id v3 = -[GTTransport_capture description](self, "description");
  v4 = -[GTTransport_capture url](self, "url");
  unsigned int v5 = -[GTTransport_capture connected](self, "connected");
  unsigned int v6 = -[GTTransport_capture invalid](self, "invalid");
  v7 = -[GTTransport_capture error](self, "error");
...
- (void)_invalidate
{
  ...
  -[GTIntKeyedDictionary_capture removeAllObjects](self->_replyHandlersMap, "removeAllObjects");
...
- (void)_dispatchMessage:(id)a3
{
  if (!LOBYTE(self->_interposerVersion))
  {
    ...
    id v6 = -[GTIntKeyedDictionary_capture countByEnumeratingWithState:objects:count:]( replyHandlersMap,  "countByEnumeratingWithState:objects:count:",  &v18,  v22,  16LL);
    if (v6)
    {
      id v7 = v6;
      ...
      do
      {
        ...
        id v7 = -[GTIntKeyedDictionary_capture countByEnumeratingWithState:objects:count:]( replyHandlersMap,  "countByEnumeratingWithState:objects:count:",  &v18,  v22,  16LL);
      }
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5 replyQueue:(id)a6 timeout:(unint64_t)a7 handler:(id)a8
{
  if ([a3 hasBeenSent])
  {
    ...
    uint64_t v31 = 34LL;
LABEL_35:
    id v32 = +[GTError_capture errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_capture,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  v31,  0LL);
    LOBYTE(v15) = 0;
    *a5 = v32;
    return v15;
  }
  ...
  if (!-[GTTransport_capture connected](self, "connected"))
  {
    if (!a5) {
      goto LABEL_36;
    }
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5
{
  return -[GTTransport_capture send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  a4,  a5,  0LL,  0LL,  0LL);
}

- (BOOL)send:(id)a3 error:(id *)a4 replyQueue:(id)a5 timeout:(unint64_t)a6 handler:(id)a7
{
  return -[GTTransport_capture send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  0LL,  a4,  a5,  a6,  a7);
}

- (BOOL)send:(id)a3 error:(id *)a4
{
  return -[GTTransport_capture send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  0LL,  a4,  0LL,  0LL,  0LL);
}

- (BOOL)sendNewMessage:(int)a3 error:(id *)a4 replyQueue:(id)a5 timeout:(unint64_t)a6 handler:(id)a7
{
  id v12 = -[GTTransportMessage_capture initWithKind:attributes:payload:]( objc_alloc(&OBJC_CLASS___GTTransportMessage_capture),  "initWithKind:attributes:payload:",  *(void *)&a3,  0LL,  0LL);
  LOBYTE(a7) = -[GTTransport_capture send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  v12,  0LL,  a4,  a5,  a6,  a7);
...
- (BOOL)sendNewMessage:(int)a3 error:(id *)a4
{
  return -[GTTransport_capture sendNewMessage:error:replyQueue:timeout:handler:]( self,  "sendNewMessage:error:replyQueue:timeout:handler:",  *(void *)&a3,  a4,  0LL,  0LL,  0LL);
}
...
- (BOOL)relayMessage:(id)a3 error:(id *)a4
{
  if ([a3 transport] == self)
  {
    ...
    uint64_t v11 = 40LL;
LABEL_15:
    id v12 = +[GTError_capture errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_capture,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  v11,  0LL);
    LOBYTE(v7) = 0;
    *a4 = v12;
    return v7;
  }
