...
- (void)dealloc
{
  ...
  -[OS_dispatch_data dealloc](&v8, sel_dealloc);
  if (v3) {
    BOOL v7 = isa == 0LL;
  }
  else {
...
- (NSString)debugDescription
{
  ...
  if (result)
  {
    -[OS_dispatch_data debugDescription].cold.1(self, result, (uint64_t *)&v4);
    return v4;
  }
...
- (uint64_t)debugDescription
{
  ...
  if (result) {
    uint64_t result = objc_msgSend(a2, "stringWithFormat:", result, object_getClassName(a1), __str);
  }
  *a3 = result;
  return result;
}
