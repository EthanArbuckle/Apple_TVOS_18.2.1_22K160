...
- (GTVMBuffer_tools_diagnostics)initWithVMBuffer:(VMBuffer *)a3
{
  ...
  v5.receiver = self;
  v5.super_class = (Class)&OBJC_CLASS___GTVMBuffer_tools_diagnostics;
  result = -[GTVMBuffer_tools_diagnostics init](&v5, "init");
  if (result) {
    result->_vmBuffer = a3;
  }
  return result;
...
{
  v7.receiver = self;
  v7.super_class = (Class)&OBJC_CLASS___GTVMBuffer_tools_diagnostics;
  v4 = -[GTVMBuffer_tools_diagnostics init](&v7, "init");
  if (v4)
  {
...
- (void)dealloc
{
  ...
  v4.receiver = self;
  v4.super_class = (Class)&OBJC_CLASS___GTVMBuffer_tools_diagnostics;
  -[GTVMBuffer_tools_diagnostics dealloc](&v4, "dealloc");
}
...
- (id)subdataWithRange:(_NSRange)a3
{
  ...
  if (a3.location + a3.length <= -[GTVMBuffer_tools_diagnostics length](self, "length")) {
    return  -[GTVMBufferSubRegion_tools_diagnostics initWithGTVMBuffer:range:]( objc_alloc(&OBJC_CLASS___GTVMBufferSubRegion_tools_diagnostics),  "initWithGTVMBuffer:range:",  self,  location,  length);
  }
  else {
    return 0LL;
  }
