...
{
  v5.receiver = self;
  v5.super_class = (Class)&OBJC_CLASS___GTFuture_replayer;
  id v2 = -[GTFuture_replayer init](&v5, "init");
  if (v2)
  {
  ...
  return (GTFuture_replayer *)v2;
}

- (void)dealloc
{
  id v3 = +[GTError_replayer errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_replayer,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  1028LL,  0LL);
  [*(id *)&self->super._private1 lock];
  if (!LOBYTE(self->_timeout))
  {
    [*(id *)&self->super._private1 unlock];
    -[GTFuture_replayer _setResult:error:notify_NOLOCK:](self, "_setResult:error:notify_NOLOCK:", 0LL, v3, 1LL);
    [*(id *)&self->super._private1 lock];
  }
...
- (void)_start_NOLOCK
{
  ...
  [*(id *)&self->super._private1 unlock];
  -[GTFuture_replayer didChangeValueForKey:](self, "didChangeValueForKey:", @"isExecuting");
}
...
- (void)waitUntilResolved
{
  if (+[GTFuture_replayer logPerformance](&OBJC_CLASS___GTFuture_replayer, "logPerformance"))
  {
...
- (BOOL)BOOLResult
{
  id v2 = -[GTFuture_replayer result](self, "result");
...
LABEL_16:
  ...
  -[GTFuture_replayer didChangeValueForKey:](self, "didChangeValueForKey:", @"isFinished");
  -[GTFuture_replayer didChangeValueForKey:](self, "didChangeValueForKey:", @"isExecuting");
}
...
- (void)cancel
{
  ...
  id v3 =  +[GTError_replayer errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_replayer,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  1027LL,  0LL);

  *(void *)&self->_resolved = v3;
  [*(id *)&self->super._private1 unlock];
  -[GTFuture_replayer _setResult:error:notify_NOLOCK:]( self,  "_setResult:error:notify_NOLOCK:",  0LL,  *(void *)&self->_resolved,  1LL);
}
...
- (void)resolveWithFuture:(id)a3
{
  if (*((_BYTE *)a3 + 24))
  {
    ...
    [*((id *)a3 + 2) unlock];
    -[GTFuture_replayer _setResult:error:notify_NOLOCK:](self, "_setResult:error:notify_NOLOCK:", v5, v6, 1LL);
  }
...
- (void)_addDependency_REQUIRESLOCK:(id)a3
{
  ...
  {
    uint64_t v7 = objc_opt_class(&OBJC_CLASS___GTFuture_replayer, v6);
...
+ (BOOL)logPerformance
{
  if (+[GTFuture logPerformance]::onceToken != -1) {
    dispatch_once(&+[GTFuture logPerformance]::onceToken, &__block_literal_global_1002);
  }
