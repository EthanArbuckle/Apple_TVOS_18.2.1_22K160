...
{
  v10.receiver = self;
  v10.super_class = (Class)&OBJC_CLASS___GTTransport_replayer;
  id v2 = -[GTTransport_replayer init](&v10, "init");
  if (v2)
  {
    ...
    *((void *)v2 + 8) = objc_opt_new(&OBJC_CLASS___NSMutableSet, v7);
    *((void *)v2 + 9) = objc_opt_new(&OBJC_CLASS___GTIntKeyedDictionary_replayer, v8);
...
- (void)dealloc
{
  ...
  v6.receiver = self;
  v6.super_class = (Class)&OBJC_CLASS___GTTransport_replayer;
...
- (id)debugDescription
{
  id v3 = -[GTTransport_replayer description](self, "description");
  v4 = -[GTTransport_replayer url](self, "url");
  unsigned int v5 = -[GTTransport_replayer connected](self, "connected");
  unsigned int v6 = -[GTTransport_replayer invalid](self, "invalid");
  uint64_t v7 = -[GTTransport_replayer error](self, "error");
...
- (NSURL)url
{
  ...
  uint64_t v9 = __Block_byref_object_copy__1099;
  objc_super v10 = __Block_byref_object_dispose__1100;
  uint64_t v11 = 0LL;
...
- (id)newSourceWithQueue:(id)a3
{
  ...
  objc_super v10 = __Block_byref_object_copy__1099;
  uint64_t v11 = __Block_byref_object_dispose__1100;
  uint64_t v12 = 0LL;
  queue = (dispatch_queue_s *)self->_queue;
...
- (void)_handleReplyTimeout:(unsigned int)a3 count:(unsigned int)a4
{
  ...
  if (v7)
  {
    if (v7[10] <= a4)
    {
      ...
      objc_msgSend( v9,  "dispatchError:",  +[GTError_replayer errorWithDomain:code:userInfo:]( GTError_replayer,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  5,  0));
    }
  }
...
- (void)_dispatchMessage:(id)a3
{
  if (!LOBYTE(self->_interposerVersion))
  {
    ...
    id v6 = -[GTIntKeyedDictionary_replayer countByEnumeratingWithState:objects:count:]( replyHandlersMap,  "countByEnumeratingWithState:objects:count:",  &v18,  v22,  16LL);
    if (v6)
    {
      id v7 = v6;
      ...
      do
      {
        ...
        id v7 = -[GTIntKeyedDictionary_replayer countByEnumeratingWithState:objects:count:]( replyHandlersMap,  "countByEnumeratingWithState:objects:count:",  &v18,  v22,  16LL);
      }
...
- (void)_scheduleInvalidation:(id)a3
{
  if (!LOBYTE(self->_interposerVersion))
  {
    ...
    rootQueue = (dispatch_queue_s *)self->_rootQueue;
    v4[0] = _NSConcreteStackBlock;
    v4[1] = 3221225472LL;
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5 replyQueue:(id)a6 timeout:(unint64_t)a7 handler:(id)a8
{
  if ([a3 hasBeenSent])
  {
    ...
    uint64_t v31 = 34LL;
LABEL_35:
    id v32 = +[GTError_replayer errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_replayer,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  v31,  0LL);
    LOBYTE(v15) = 0;
    *a5 = v32;
    return v15;
  }
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5
{
  return -[GTTransport_replayer send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  a4,  a5,  0LL,  0LL,  0LL);
}

- (BOOL)send:(id)a3 error:(id *)a4 replyQueue:(id)a5 timeout:(unint64_t)a6 handler:(id)a7
{
  return -[GTTransport_replayer send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  0LL,  a4,  a5,  a6,  a7);
}

- (BOOL)send:(id)a3 error:(id *)a4
{
  return -[GTTransport_replayer send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  a3,  0LL,  a4,  0LL,  0LL,  0LL);
}

- (BOOL)sendNewMessage:(int)a3 error:(id *)a4 replyQueue:(id)a5 timeout:(unint64_t)a6 handler:(id)a7
{
  id v12 = -[GTTransportMessage_replayer initWithKind:attributes:payload:]( objc_alloc(&OBJC_CLASS___GTTransportMessage_replayer),  "initWithKind:attributes:payload:",  *(void *)&a3,  0LL,  0LL);
  LOBYTE(a7) = -[GTTransport_replayer send:inReplyTo:error:replyQueue:timeout:handler:]( self,  "send:inReplyTo:error:replyQueue:timeout:handler:",  v12,  0LL,  a4,  a5,  a6,  a7);
...
- (BOOL)sendNewMessage:(int)a3 error:(id *)a4
{
  return -[GTTransport_replayer sendNewMessage:error:replyQueue:timeout:handler:]( self,  "sendNewMessage:error:replyQueue:timeout:handler:",  *(void *)&a3,  a4,  0LL,  0LL,  0LL);
}
...
- (BOOL)relayMessage:(id)a3 error:(id *)a4
{
  if ([a3 transport] == self)
  {
    ...
    uint64_t v11 = 40LL;
LABEL_15:
    id v12 = +[GTError_replayer errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_replayer,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  v11,  0LL);
    LOBYTE(v7) = 0;
    *a4 = v12;
    return v7;
  }
