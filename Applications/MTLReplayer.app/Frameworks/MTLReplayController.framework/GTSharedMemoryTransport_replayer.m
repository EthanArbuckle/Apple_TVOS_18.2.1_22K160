...
- (GTSharedMemoryTransport_replayer)initWithMode:(int)a3
{
  return -[GTSharedMemoryTransport_replayer initWithMode:initialMessageSerial:]( self,  "initWithMode:initialMessageSerial:",  *(void *)&a3,  0LL);
}

- (GTSharedMemoryTransport_replayer)initWithMode:(int)a3 initialMessageSerial:(unsigned int)a4
{
  v15.receiver = self;
  v15.super_class = (Class)&OBJC_CLASS___GTSharedMemoryTransport_replayer;
  ...
  id v11 = -[GTSharedMemoryTransport_replayer methodForSelector:](v7, "methodForSelector:");
...
LABEL_9:
  ...
  v7->_bufferedMessages = (NSMutableArray *)objc_opt_new(&OBJC_CLASS___NSMutableArray, v12);
...
{
  v3.receiver = self;
  v3.super_class = (Class)&OBJC_CLASS___GTSharedMemoryTransport_replayer;
...
- (BOOL)send:(id)a3 inReplyTo:(id)a4 error:(id *)a5 replyQueue:(id)a6 timeout:(unint64_t)a7 handler:(id)a8
{
  if (self->_deferred)
  {
    v13 = objc_alloc_init(&OBJC_CLASS___GTBufferedSendOperation_replayer);
...
- (void)_mapSharedMemoryFile:(int)a3 size:(unint64_t)a4 error:(id *)a5
{
  size_t v8 = -[GTSharedMemoryTransport_replayer _calculateVMRegionMapSize:](self, "_calculateVMRegionMapSize:", a4);
...
- (BOOL)_initializeSMRegion:(sm_region_header *)a3 size:(unint64_t)a4 name:(const char *)a5 error:(id *)a6
{
  ...
  {
    uint64_t v12 = -[GTSharedMemoryTransport_replayer _mapSharedMemoryFile:size:error:]( self,  "_mapSharedMemoryFile:size:error:",  v11,  v9,  a6);
    if (v12)
    {
      v13 = v12;
      close(v11);
...
- (BOOL)_openSMRegion:(sm_region_header *)a3 size:(unint64_t)a4 name:(const char *)a5 error:(id *)a6
{
  ...
  id v16 = -[GTSharedMemoryTransport_replayer _mapSharedMemoryFile:size:error:]( self,  "_mapSharedMemoryFile:size:error:",  v12,  v15,  a6);
  if (v16)
  {
    v17 = v16;
    close(v14);
...
- (BOOL)_createAndRunSources:(id *)a3
{
  ...
  v13 = __Block_byref_object_copy__1099;
  int v14 = __Block_byref_object_dispose__1100;
  uint64_t v15 = 0LL;
  v4 = objc_alloc(&OBJC_CLASS___GTContinuation_replayer);
...
- (BOOL)_serverConnect:(id *)a3
{
  if (!-[GTSharedMemoryTransport_replayer _initializeSMRegion:size:name:error:]( self,  "_initializeSMRegion:size:name:error:",  &self->_outgoingShmem,  self->_sm_region_size_small,  self->_sendName,  a3)
    || !-[GTSharedMemoryTransport_replayer _initializeSMRegion:size:name:error:]( self,  "_initializeSMRegion:size:name:error:",  &self->_incomingShmem,  self->_sm_region_size_large,  self->_receiveName,  a3))
  {
    return 0;
  }

  -[GTSharedMemoryTransport_replayer _setupIOBuffers](self, "_setupIOBuffers");
  BOOL result = -[GTSharedMemoryTransport_replayer _createAndRunSources:](self, "_createAndRunSources:", a3);
...
- (BOOL)_clientConnect:(id *)a3
{
  if (!-[GTSharedMemoryTransport_replayer _openSMRegion:size:name:error:]( self,  "_openSMRegion:size:name:error:",  &self->_incomingShmem,  self->_sm_region_size_small,  self->_receiveName,  a3)
    || !-[GTSharedMemoryTransport_replayer _openSMRegion:size:name:error:]( self,  "_openSMRegion:size:name:error:",  &self->_outgoingShmem,  self->_sm_region_size_large,  self->_sendName,  a3))
  {
    return 0;
  }
...
- (id)connect
{
  id v3 = +[GTFuture_replayer future](&OBJC_CLASS___GTFuture_replayer, "future");
  queue = (dispatch_queue_s *)self->super.super._queue;
...
- (int64_t)_write:(const void *)a3 size:(unint64_t)a4
{
  ...
  if (a4)
  {
    ...
    unint64_t v8 = -[GTSharedMemoryTransport_replayer _computeBytesAvailableToWrite](self, "_computeBytesAvailableToWrite");
    if (v8 > 4)
    {
      if (v8 - 4 < v4) {
        unint64_t v4 = v8 - 4;
      ...
      int v12 = v4 + 4;
      -[GTSharedMemoryTransport_replayer _copyToSM:size:](self, "_copyToSM:size:", &v12, 4LL);
...
- (void)_dequeuePacket
{
  unint64_t v3 = -[GTSharedMemoryTransport_replayer _computeBytesAvailableToRead](self, "_computeBytesAvailableToRead");
  if (v3 <= 3)
  {
    unsigned int v5 = objc_autoreleasePoolPush();
    -[GTBaseStreamTransport_replayer _scheduleInvalidation:]( self,  "_scheduleInvalidation:",  +[GTError_replayer errorWithDomain:code:userInfo:]( &OBJC_CLASS___GTError_replayer,  "errorWithDomain:code:userInfo:",  @"DYErrorDomain",  259LL,  0LL));
  ...
  {
    unint64_t v4 = v3;
    -[GTSharedMemoryTransport_replayer _copyFromSM:size:](self, "_copyFromSM:size:", &v7, 4LL);
    if (v4 < v7)
    {
